...
public class ItemInfo : IEnumerable<ItemData>
{
        ...
        IEnumerable<CodeInstruction> instructionsEnumerable)
    {
        ...
        CodeInstruction lastBranch = instructions.Reverse()
            .First(i => i.Branches(out Label? label) && targetedInstr.labels.Contains(label!.Value));
    ...
    {
        MethodInfo itemDeleter = AccessTools.DeclaredMethod(typeof(Inventory), nameof(Inventory.RemoveItem),
    ...
    static ItemInfo()
    {
        ...
        harmony.Patch(AccessTools.DeclaredMethod(typeof(Inventory), nameof(Inventory.Save)),
            prefix: new HarmonyMethod(AccessTools.DeclaredMethod(typeof(ItemInfo), nameof(SaveInventoryPrefix)),
                Priority.First));
        foreach (MethodInfo method in typeof(ItemDrop.ItemData)
                     .GetMethods(BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public |
                                 BindingFlags.NonPublic).Where(m => m.Name == nameof(ItemDrop.SaveToZDO)))
        {
            harmony.Patch(method,
                prefix: new HarmonyMethod(AccessTools.DeclaredMethod(typeof(ItemInfo), nameof(SavePrefix)),
                    Priority.First));
        }
        ...
        foreach (MethodInfo method in typeof(ItemDrop.ItemData)
                     .GetMethods(BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public |
                                 BindingFlags.NonPublic).Where(m => m.Name == nameof(ItemDrop.LoadFromZDO)))
        {
            harmony.Patch(method,
                postfix: new HarmonyMethod(AccessTools.DeclaredMethod(typeof(ItemInfo),
...
public class ForeignItemInfo : IEnumerable<object>
{
    ...
    public Dictionary<string, T> GetAll<T>() where T : class =>
        call(nameof(GetAll), Array.Empty<object?>(), Array.Empty<Type?>(), typeof(T)) as T as Dictionary<string, T> ??
